...
struct HttpRequest{
    ...
    char response_header_buffer[1024]; //响应头部 buffer
};
...
void line_read_from_buffer(struct HttpRequest *request){
    ...
    if(end_p == NULL){
        ...
        if(request->buffer[request->buffer_write_index - 1] == '\r'){
            //结尾是 \r，将这个字符保留在 buffer 中，其余的字符写入到 line 中
            copy_count = ((request->buffer_write_index - 1)- request->buffer_read_index)/sizeof(char);
            strncpy(request->line + request->line_write_index, request->buffer + request->buffer_read_index, copy_count);
            ...
            request->buffer_write_index = 1;
            //更新 line_write_index
            request->line_write_index += copy_count;
        }else{
...
void do_response(struct HttpRequest *request){
    ...
    case GET:
        ...
        memset(request->response_header_buffer, 0, sizeof(request->response_header_buffer));
        ...
        //写入请求行    
        sprintf(request->response_header_buffer + strlen(request->response_header_buffer), "%s\r\n", "HTTP/1.1 200 OK");

        //写入头部
        if(strstr(request->url, ".jpg") != NULL){
            sprintf(request->response_header_buffer + strlen(request->response_header_buffer), "Content-type:%s\r\n", "image/jpeg");
        }else if(strstr(request->url, ".png") != NULL){
            sprintf(request->response_header_buffer + strlen(request->response_header_buffer), "Content-type:%s\r\n", "image/png");
        }
        ...
        if((fp = fopen(request->filename, "rb")) == NULL){
        ...
        fseek(fp, 0, SEEK_END);
        int length = ftell(fp);
        sprintf(request->response_header_buffer + strlen(request->response_header_buffer), "Content-Length:%ld\r\n\r\n", ftell(fp));

        write(request->clientfd, request->response_header_buffer, strlen(request->response_header_buffer));

        //文件指针移回头部
        rewind(fp);

        int freadn;
        while( (freadn = fread(file_buffer, sizeof(char),sizeof(file_buffer), fp)) != 0){
            int writen = write(request->clientfd, file_buffer, freadn);
